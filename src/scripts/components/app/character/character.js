...
let Character = class Character {
    ...
    exportPony() {
        ...
        file_saver_1.saveAs(new Blob([data], { type: 'text/plain;charset=utf-8' }), `${this.pony.name}.txt`);
    }
    exportPonies() {
        const data = this.ponies.map(ponyToExport).join('\r\n') + '\r\n';
        file_saver_1.saveAs(new Blob([data], { type: 'text/plain;charset=utf-8' }), 'ponies.txt');
    }
