...
exports.engines = [
    { name: '默认', engine: interfaces_1.Engine.Default },
    { name: '分层地块', engine: interfaces_1.Engine.LayeredTiles },
    { name: '白色', engine: interfaces_1.Engine.Whiteness },
];
...
let PonyTownGame = class PonyTownGame {
    ...
    draw() {
        ...
        if (!this.socket || !this.socket.isConnected) {
            this.drawMessage(this.webgl, '连接中...');
        }
        else if (!this.loaded) {
            if (this.placeInQueue) {
                this.drawMessage(this.webgl, `排队中 (${this.placeInQueue})`);
            }
            else {
                this.drawMessage(this.webgl, '加载中...');
            }
        }
        else if ((performance.now() - this.socket.lastPacket) > CONNECTION_ISSUE_TIMEOUT) {
            // this.drawMessage('连接问题...');
        }
