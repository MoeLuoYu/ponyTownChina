...
const hidePlayerLimit = '无法隐藏更多玩家。';
const cannotHidePlayerInParty = '无法隐藏队伍中的玩家。';
const cannotHideFriends = '无法隐藏好友。';
const unhideAllLimit = '无法取消隐藏玩家，请稍后再试。';
const unhideAllLimitNote = '每小时只能执行一次。';
...
class HidingService {
    ...
    requestHide(requester, target, timeout) {
        ...
        if (requester.accountId === target.accountId) {
            chat_1.saySystem(requester, `你不能隐藏你自己`);
        }
    ...
    requestUnhideAll(requester) {
        ...
        if (unhideTimestamp > Date.now()) {
            ...
            this.notifications.addNotification(requester, {
                id: 0,
                name: '',
                message: '您确定要取消对所有临时隐藏玩家的隐藏吗？',
                note: '每小时只能执行一次，此操作需要重新进入游戏。',
                flags: 2 /* Yes */ | 4 /* No */,
    ...
    unhideAll(byClient) {
        ...
        this.log(logger_1.systemMessage(by, '取消隐藏所有'));
    }
    merged(target, merge) {
