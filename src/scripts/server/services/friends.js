...
class FriendsService {
    ...
    add(client, target) {
        ...
        if (can === 4 /* LimitReached */) {
            return chat_1.saySystem(client, '已达到请求拒绝限制');
        }
        else if (can !== 0 /* Yes */) {
            return chat_1.saySystem(client, '无法发送请求');
        }
        const pending = this.pending.get(client.accountId) || new Set();
        if (pending.has(target.accountId))
            return chat_1.saySystem(client, '已发送请求');
        if (isFriend(client, target))
            return chat_1.saySystem(client, '已在好友列表中');
        if (client.friends.size >= constants_1.FRIENDS_LIMIT)
            return chat_1.saySystem(client, '你的好友列表已满');
        if (target.friends.size >= constants_1.FRIENDS_LIMIT)
            return chat_1.saySystem(client, '目标玩家好友列表已满');
        ...
        if (pending.size >= exports.PENDING_LIMIT)
            return chat_1.saySystem(client, '待处理请求过多');
    ...
    acceptInvitation(client, friend, notificationId) {
        ...
        chat_1.saySystem(client, `好友请求被 ${entityUtils_1.getEntityName(friend.pony, client)} 接受`);
    ...
    rejectInvitation(client, friend, notificationId) {
        ...
        chat_1.saySystem(client, `好友请求被 ${entityUtils_1.getEntityName(friend.pony, client)} 拒绝`);
        this.removePending(client, friend);
        this.notificationService.removeNotification(friend, notificationId);
