...
exports.createUpdateAccount = (findAccount, log) => async (account, update) => {
    ...
    if (update) {
        ...
        if (fixed.name && fixed.name !== a.name) {
            up.name = fixed.name;
            log(a._id, `重命名 "${a.name}" => "${fixed.name}"`);
        }
        if (fixed.birthdate) {
            ...
            if ((date && a.birthdate && date.getTime() !== a.birthdate.getTime()) || !a.birthdate) {
                up.birthdate = date;
                const from = a.birthdate ? `${utils_1.formatISODate(a.birthdate)} (${adminUtils_1.getAge(a.birthdate)}岁)` : `未定义`;
                const to = up.birthdate ? `${utils_1.formatISODate(up.birthdate)} (${adminUtils_1.getAge(up.birthdate)}岁)` : `未定义`;
                log(a._id, `更改出生日期 ${from} => ${to}`);
            }
        }
        Object.assign(a, up);
...
exports.createRemoveSite = (findAuth, countAllVisibleAuths, log) => async (account, siteId) => {
    ...
    if (!auth || auth.disabled) {
        throw new userError_1.UserError('社交账号未找到');
    }
    else if (auths === 1) {
        throw new userError_1.UserError('无法移除你唯一的一个社交账号');
    }
    else {
        log(account._id, `移除认证: ${auth.name} [${auth._id}]`);
