...
async function merge(id, withId, reason, removedDocument, allowAdmin = false, creatingDuplicates = false) {
    ...
    logger_1.system(account._id, `合并 ${account.name} 与 ${merge.name} [${merge._id}] (${reason}) (${Date.now() - start}ms)`);
}
...
async function split(accountId, mergeId, split, keep, reason) {
    ...
    logger_1.system(account._id, `分离出 ${unmerge.name} [${unmerge._id}] (${reason}) (${Date.now() - start}ms)`);
}
exports.split = split;
