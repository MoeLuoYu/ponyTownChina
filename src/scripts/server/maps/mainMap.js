...
function createToyStash(x, y) {
    return [
        ...
        controllerUtils_1.createSign(x, y, '礼物堆', (_, client) => entityUtils_1.updateEntityOptions(client.pony, playerUtils_1.getNextToyOrExtra(client)), entities.sign),
    ];
}
...
function addSeasonalObjects(world, map, season, holiday) {
    ...
    const addJackoLanternSpot = (x, y) => {
        const giveLantern = controllerUtils_1.give(entities.jackoLanternOn.type, '现在去收集一些糖果吧！');
        add(controllerUtils_1.createSign(x, y, '南瓜灯', giveLantern, entities.signQuest));
    ...
    if (isHalloween) {
        ...
        add(controllerUtils_1.createSign(donateX, donateY, '捐赠糖果', donateCandy, entities.signDonate));
    ...
    if (isChristmas) {
        ...
        const donateX = 64, donateY = 79;
        add(controllerUtils_1.createSign(donateX, donateY, '捐赠礼物', donateGift, entities.signDonate));
    ...
    if (isEaster) {
        ...
        add(controllerUtils_1.createSign(74.00, 73.80, '彩蛋篮子', giveBasket, entities.signQuest));
        // spot 2
        ...
        add(controllerUtils_1.createSign(34 + 70, 33.8 + 53, '彩蛋篮子', giveBasket, entities.signQuest));
        // donation spot
        add(controllerUtils_1.createSign(62.00, 78.00, '捐赠彩蛋', donateEgg, entities.signDonate));
...
function createMainMap(world) {
    ...
    function add(entity) {
        if (entity.x < 0 || entity.x > map.width || entity.y < 0 || entity.y > map.height) {
            if (DEVELOPMENT) {
                logger_1.logger.warn(`跳过了实体 (${entities_1.getEntityTypeName(entity.type)}) 超出地图范围 (${entity.x} ${entity.y})`);
            }
    ...
    add(controllerUtils_1.createSignWithText(pierX + 8.5, pierY + 71.1, '派对岛', `跳上小船，前往专属于你的派对岛屿`));
    ...
    add(controllerUtils_1.createSignWithText(70.5, 70.5, '小马镇', '小马镇\n[建设中]', entities.sign));
    ...
    const addCat = (x, y) => {
        ...
        entity.serverUpdate = delta => {
            ...
            if (hideDelay < 0 && delay < 0) {
                ...
                if (rand < 0.1) {
                    chat_1.sayToAll(entity, 'meow', '喵~', 1 /* System */, {});
                    delay = lodash_1.random(2, 4, true);
                }
                else if (rand < 0.5) {
    ...
    add(controllerUtils_1.createSign(70, 61.5, '信件告示牌', controllerUtils_1.give(entities.letter.type, `这是你的信！`), entities.sign));
    ...
    add(controllerUtils_1.createSign(77.06, 60.16, '玫瑰告示牌', controllerUtils_1.give(entities.rose.type, `这是你的玫瑰！`), entities.sign));
    if (world.featureFlags.test) {
        add(controllerUtils_1.createSign(14.5, 70, '公共岛屿', (_, client) => world_1.goToMap(world, client, 'public-island'), entities.signDebug));
        add(controllerUtils_1.createSignWithText(60.7, 60.2, '可拾取物品', `点击物品即可携带`, entities.signDebug));
    ...
    if (BETA) {
        ...
        add(controllerUtils_1.createSign(62.7, 58.2, '南瓜灯', controllerUtils_1.give(entities.jackoLanternOn.type, '拿个灯笼'), entities.signDebug));
        add(controllerUtils_1.createSign(62.7, 60.2, '可拾取物体', (_, client) => playerUtils_1.holdItem(client.pony, lodash_1.sample(objects)), entities.signDebug));
        add(controllerUtils_1.createSign(77.0, 69.0, '调色板', (_, client) => world_1.goToMap(world, client, 'palette'), entities.signDebug));
    }
    ...
    if (true) {
        const toLake = { icon: 8 /* Lake */, name: '湖泊' };
        const toHarbor = { icon: 4 /* Boat */, name: '港口' };
        const toSpawn = { icon: 0 /* Spawn */, name: '出生点' };
        const toTownCenter = { icon: 2 /* TownCenter */, name: '镇中心' };
        const toPineForest = { icon: 3 /* PineForest */, name: '松树林' };
        const toPartyIsland = { icon: 4 /* Boat */, name: '派对岛' };
        const toGiftPile = { icon: 6 /* GiftPile */, name: '礼物堆' };
        const toMountains = { icon: 5 /* Mountains */, name: '山脉' };
        const toForest = { icon: 7 /* Forest */, name: '森林' };
        const toPumpkinFarm = { icon: 1 /* Pumpkins */, name: '南瓜农场' };
        const toFlowerField = { icon: 12 /* Fields */, name: '花田' };
        const toBarrelStorage = { icon: 11 /* Barrels */, name: '木桶仓库' };
        const toMines = { icon: 10 /* Mines */, name: '矿场' };
        const toBridge = { icon: 9 /* Bridge */, name: '桥' };
        const toCarrots = { icon: 13 /* Carrots */, name: '胡萝卜农场' };
