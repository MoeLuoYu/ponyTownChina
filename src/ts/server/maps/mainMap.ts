...
function createToyStash(x: number, y: number) {
	return [
		...
		createSign(x, y, '礼物堆', (_, client) => updateEntityOptions(client.pony, getNextToyOrExtra(client)), entities.sign),
	];
}
...
function addSeasonalObjects(world: World, map: ServerMap, season: Season, holiday: Holiday) {
	...
	const addJackoLanternSpot = (x: number, y: number) => {
		const giveLantern = give(entities.jackoLanternOn.type, '现在去收集一些糖果吧！');

		add(createSign(x, y, '南瓜灯', giveLantern, entities.signQuest));
	...
	if (isHalloween) {
		...
		add(createSign(donateX, donateY, '捐赠糖果', donateCandy, entities.signDonate));
	...
	if (isChristmas) {
		...
		const donateX = 64, donateY = 79;
		add(createSign(donateX, donateY, '捐赠礼物', donateGift, entities.signDonate));
	...
	if (isEaster) {
		...
		add(createSign(74.00, 73.80, '彩蛋篮子', giveBasket, entities.signQuest));
		...
		add(createSign(34 + 70, 33.8 + 53, '彩蛋篮子', giveBasket, entities.signQuest));

		// donation spot
		add(createSign(62.00, 78.00, '捐赠彩蛋', donateEgg, entities.signDonate));
...
export function createMainMap(world: World): ServerMap {
	...
	function add(entity: ServerEntity) {
		if (entity.x < 0 || entity.x > map.width || entity.y < 0 || entity.y > map.height) {
			if (DEVELOPMENT) {
				logger.warn(`跳过了实体 (${getEntityTypeName(entity.type)}) 超出地图范围 (${entity.x} ${entity.y})`);
			}
	...
	add(createSignWithText(pierX + 8.5, pierY + 71.1, '派对岛',
		`跳上小船，前往专属于你的派对岛屿`));
	...
	add(createSignWithText(70.5, 70.5, '小马镇', '小马镇\n[建设中]', entities.sign));
	...
	const addCat = (x: number, y: number) => {
		...
		entity.serverUpdate = delta => {
			...
			if (hideDelay < 0 && delay < 0) {
				...
				if (rand < 0.1) {
					sayToAll(entity, 'meow', '喵~', MessageType.System, {});
					delay = random(2, 4, true);
				} else if (rand < 0.5) {
	...
	add(createSign(70, 61.5, '信件告示牌', give(entities.letter.type, `这是你的信！`), entities.sign));
	...
	add(createSign(77.06, 60.16, '玫瑰告示牌', give(entities.rose.type, `这是你的玫瑰！`), entities.sign));

	if (world.featureFlags.test) {
		add(createSign(14.5, 70, '公共岛屿', (_, client) => goToMap(world, client, 'public-island'), entities.signDebug));
		add(createSignWithText(60.7, 60.2, '可拾取物品', `点击物品即可携带`, entities.signDebug));
	...
	if (BETA) {
		...
		add(createSign(62.7, 58.2, '南瓜灯', give(entities.jackoLanternOn.type, '拿个灯笼'), entities.signDebug));
		add(createSign(62.7, 60.2, '可拾取物体', (_, client) => holdItem(client.pony, sample(objects)!), entities.signDebug));
		add(createSign(77.0, 69.0, '调色板', (_, client) => goToMap(world, client, 'palette'), entities.signDebug));
	}
	...
	if (true) {
		const toLake = { icon: SignIcon.Lake, name: '湖泊' };
		const toHarbor = { icon: SignIcon.Boat, name: '港口' };
		const toSpawn = { icon: SignIcon.Spawn, name: '出生点' };
		const toTownCenter = { icon: SignIcon.TownCenter, name: '镇中心' };
		const toPineForest = { icon: SignIcon.PineForest, name: '松树林' };
		const toPartyIsland = { icon: SignIcon.Boat, name: '派对岛' };
		const toGiftPile = { icon: SignIcon.GiftPile, name: '礼物堆' };
		const toMountains = { icon: SignIcon.Mountains, name: '山脉' };
		const toForest = { icon: SignIcon.Forest, name: '森林' };
		const toPumpkinFarm = { icon: SignIcon.Pumpkins, name: '南瓜农场' };
		const toFlowerField = { icon: SignIcon.Fields, name: '花田' };
		const toBarrelStorage = { icon: SignIcon.Barrels, name: '木桶仓库' };
		const toMines = { icon: SignIcon.Mines, name: '矿场' };
		const toBridge = { icon: SignIcon.Bridge, name: '桥' };
		const toCarrots = { icon: SignIcon.Carrots, name: '胡萝卜农场' };
