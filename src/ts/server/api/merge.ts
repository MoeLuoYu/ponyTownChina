...
async function merge(
	...
	system(account._id, `合并 ${account.name} 与 ${merge.name} [${merge._id}] (${reason}) (${Date.now() - start}ms)`);
}
...
export async function split(
	...
	system(account._id, `分离出 ${unmerge.name} [${unmerge._id}] (${reason}) (${Date.now() - start}ms)`);
}
