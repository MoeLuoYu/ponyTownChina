			...
			if (update) {
				...
				if (fixed.birthdate) {
					...
					if ((date && a.birthdate && date.getTime() !== a.birthdate.getTime()) || !a.birthdate) {
						...
						const from = a.birthdate ? `${formatISODate(a.birthdate)} (${getAge(a.birthdate)}岁)` : `未定义`;
						const to = up.birthdate ? `${formatISODate(up.birthdate)} (${getAge(up.birthdate)}岁)` : `未定义`;
	...
	(findAuth: FindAuth, countAllVisibleAuths: CountAuths, log: LogAccount) =>
		async (account: IAccount, siteId: unknown) => {
			...
			if (!auth || auth.disabled) {
				throw new UserError('社交账号未找到');
			} else if (auths === 1) {
				throw new UserError('无法移除你唯一的一个社交账号');
			} else {
