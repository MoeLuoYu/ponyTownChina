...
if (args.login || args.admin) {
	passport.serializeUser((account: any, done: any) => done(null, account._id.toString()));
	passport.deserializeUser((id: any, done: any) =>
		Account.findById(id, (err: any, a: any) => done(err, a && !isBanned(a) ? a : false)));
}
...
if (args.login) {
	...
	const csp = `object-src 'none';`
		...
		+ `script-src 'self' 'unsafe-eval' ${script} ${scriptES} ${analytics};`
		// + `report-uri /api2/csp`
		;
